...
def loadDatabase():
    ...
    for IDC in IDCs:
        ...
        for dataCenterIndex in range(0, dataCenterCount):
            dataCenterTuple = generateIDCTuple(
                IDC, IDC["data_centers"][dataCenterIndex])
            cursor.execute(
                "SELECT id FROM data_centers WHERE ip = ? and idc = ?",
                (IDC["data_centers"][dataCenterIndex]['network_info']['ip'],
                 IDC["idc"]))
            if cursor.fetchone() is None:
                dataCenters.append(dataCenterTuple)
    saveIDC(IDCs)
    cursor.executemany(
        'INSERT INTO data_centers VALUES (null,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
        dataCenters)
    conn.commit()
...
def go_ping(pingType, sqlWhere, pingCount, pingTimeout):
    ...
    ping(pingType,
         ("SELECT ip,tcping_port,enable_tcping_test FROM data_centers " +
          sqlWhere).encode("utf-8"), pingCount, pingTimeout)
...
def generateIDCTuple(idc, dataCenter):
    ...
    if "ip" not in dataCenter["network_info"]:
        if canDownload:
            dataCenter["network_info"]["ip"] = getIPFromHost(
                getHostFromUrl(dataCenter["network_info"]["test_file_link"]))
        else:
            print(idc["idc"], "的数据中心缺少网络信息。")
            sys.exit(1)
    if "tcping_port" not in dataCenter["network_info"]:
        if canDownload:
            dataCenter["network_info"]["tcping_port"] = 80 if dataCenter[
                "network_info"]["test_file_link"][4] == ':' else 443
        else:
            canTcping = False
    dataCenter["conf"] = {}
    ...
    return (idc["idc"], generateIDCAbbr(idc["idc_abbr"]),
            idc["official_website"], dataCenter["conf"]["enable_tcping_test"],
            dataCenter["conf"]["enable_download_test"],
            dataCenter["geo_info"]["country"], dataCenter["geo_info"]["city"],
            dataCenter["network_info"]["ip"],
            dataCenter["network_info"]["tcping_port"],
            dataCenter["network_info"]["test_file_link"],
            dataCenter["ping_status"]["ping_loss"],
            dataCenter["ping_status"]["ping_received"],
            dataCenter["ping_status"]["ping_time"],
            dataCenter["speed_status"]["download_speed"])
