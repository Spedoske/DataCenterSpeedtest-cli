import socket, requests, json, util
...
for IDC in IDCs:
    if IDC['idc'] or IDC['localized_idc'] in sys.argv:
        ...
        for dataCenterIndex in range(0, dataCenterCount):
            ...
            localized_idc = IDC['localized_idc'] + ' ' + localized_data_center_array[dataCenterIndex]
            ip = util.getIPFromHost(util.getHostFromUrl(url))
            if ip not in smartpingSetting['Network'] and ip != None:
                smartpingSetting['Network'][ip] = getDefaultEndPoint(localized_idc, ip)
postres = requests.post(
    smartpingUrl + "/api/saveconfig.json",
    data={
