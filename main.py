...
parser.add_argument("--list", nargs='+', help="输出筛选后的结果,需要提供key")
...
def handleFlush():
    if args.flushPing:
        cursor.execute(
            "UPDATE data_centers SET ping_loss=0,ping_received=0,ping_time=0 "
            + args.search)
        conn.commit()
    if args.flushSpeedtest:
        cursor.execute("UPDATE data_centers SET download_speed=0 " +
                       args.search)
        conn.commit()
...
def handleSpeedtest():
    if args.speedtest:
        ...
        result = cursor.execute(
            'SELECT idc,country,city,test_file_link,enable_download_test,download_speed FROM data_centers '
            + args.search).fetchall()
        count = len(result)
        for row in result:
            if row[4] == 1:
                if row[3] in testFileLinkSet:
                    print(
                        "节点:", row[0] + " " + row[1] +
                        "({0}/{1})".format(currentCount + 1, count))
                    print("测试文件地址:", row[3])
                    print("已测速, 跳过...")
                else:
                    [time, data,
                     withError] = dataCenterSpeedtest.measureDownloadSpeed(
                         row[0], row[1] + " " + row[2], row[3], currentCount,
                         count)
                    cursor.execute(
                        "UPDATE data_centers SET download_speed=? WHERE test_file_link=?",
                        (max(data / time, row[5]), row[3]))
                    conn.commit()
                    if not withError:
                        testFileLinkSet.add(row[3])
            else:
                print(
...
TranslateMap = [
    "序号", "服务商名称", "服务商缩写", "官方网站", "是否可以进行tcping", "是否可以进行测速", "国家", "城市",
    "IP地址", "tcping端口", "下载文件链接", "ping丢包", "ping收包", "ping的平均时间", "下载速度"
]
...
def handleList():
    ...
    results = cursor.execute(
        'SELECT *,CASE WHEN ping_received+ping_loss=1 THEN 0.0 ELSE (CAST(ping_loss as REAL)/(ping_received+ping_loss)) END as ping_loss_rate FROM data_centers '
        + args.search + " " + args.order).fetchall()
    ...
    for result in results:
        ...
        for listArg in listArgs:
            if listArg == 11:
                resultArray.append("{}({:.1f}%)".format(
                    result[listArg],
                    util.getPercentage(result[11], result[11] + result[12])))
            elif listArg == 12:
                resultArray.append("{}({:.1f}%)".format(
                    result[listArg],
                    util.getPercentage(result[12], result[11] + result[12])))
            elif listArg == 13:
